...
export class StripeProvider {
  ...
  async updatePlanToPro(stripeCustomerId: string, userId: string): Promise<string | null> {
    try {
      ...
      const session = await this.stripe.checkout.sessions.create({
        ...
        cancel_url: `${domainUrl}/subscription-aborted`,
        mode: 'subscription',
      ...
      Logger.error(`StripeService (updatePlanToPRO): ${error.message}`);
      throw new Error(error.message);
    }
  }
  ...
  async webhookHandler(payload: Buffer<ArrayBuffer>, sig: string) {
    ...
    //'customer.subscription.deleted' pode ser tratado futuramente para cancelamento direto no Dashboard do Stripe ou por falta de pagamento
    ...
    try {
      const subscriptionData = await this.handleStripeEvent(event);
  ...
  async handleStripeEvent(event: Stripe.Event) {
    switch (event.type) {
      ...
      //Não utilizarei neste contexto
      case 'customer.subscription.deleted':
        const subscription = event.data.object as Stripe.Subscription;
        Logger.info(`⚠️ Subscription canceled: ${subscription.id}`);
      // Lógica de DB aqui... para cancelamento diretamente no Deshboard do Stripe ou por falta de pagamento
  ...
  async cancelSubscription(stripeSubscriptionId: string) {
    try {
      ...
      return {
        stripeCustomerId: subscriptionCanceled.customer as string,
